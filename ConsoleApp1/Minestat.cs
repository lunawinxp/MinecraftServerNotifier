    ...
    public class MineStat
    {
        ...
        private ConnStatus ParseJsonProtocolPayload(byte[] rawPayload)
        {
            try
            {
                ...
                // get online players sample, this is the worst way of doing this, but i do not use XElement / JsonReaderWriterFactory so i just, json.net boom
                // very messy, probably prone to failiure in some cases but oh well 
                // https://github.com/lunalunaaaa 
                CurrentPlayersList = "";
                try
                {
                    var jsonstr = JsonConvert.SerializeXNode(root);
                    dynamic dynamicJson = JObject.Parse(jsonstr);
                    string fullPlayerList = "";
                    if (CurrentPlayersInt > 1)
                    {
                        foreach (var itm in dynamicJson.root.players.sample.item)
                        {
                            fullPlayerList += itm.name["#text"] + ",";
                        }
                    }
                    else
                    {
                        fullPlayerList = dynamicJson.root.players.sample.item.name["#text"] + ",";
                    }
                    CurrentPlayersList = fullPlayerList;
                }
                catch
                {
